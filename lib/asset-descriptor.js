...
function isValidPoolId(poolId) {
  if (typeof poolId === 'string') return /^\w{64}$/.test(poolId) || stellar_base_.StrKey.isValidLiquidityPool(poolId);
...
class LiquidityPoolDescriptor extends AssetDescriptor {
  constructor(id) {
    super();
    this.poolId = id.startsWith('L') ? id : stellar_base_.StrKey.encodeLiquidityPool(Buffer.from(id, 'hex'));
    this.poolType = 0;
    this.type = 3;
    Object.freeze(this);
  }
